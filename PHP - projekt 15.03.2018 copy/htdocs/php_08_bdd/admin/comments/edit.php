<?php require_once '/../../scripts/bootstrap/database.php'; ?>
<?php require_once '/../../scripts/bootstrap.php';
?>


<?php include '/../layout/header.php'; ?>
<?php
$com = $database->getObjectById('comments', $_GET['id']);
if ($com->num_rows > 0) {
    while ($row = $com->fetch_assoc()):
        ?>
        <form action="../../scripts/comments/edit.php" method="post">
            <div class="col-md-12 col-sm-12 col-xs-12 no-padding">
                <p class="page-title">Edytuj komentarz</p>
                <?php
                if (isset($_GET['status']) && $_GET['status'] == "error") {
                    echo '<div class="alert alert-danger" role="alert">Komentarz nie została zmieniony.</div>';
                }
                if (isset($_GET['status']) && $_GET['status'] == "ok") {
                    echo '<div class="alert alert-success" role="alert">Komentarz został poprawnie zmieniony.</div>';
                }
                ?>
            </div>
            <div class="col-md-12 col-sm-12 col-xs-12 single-form-element">
                <input type="text" name="author" placeholder="Author" class="form-control"
                       value="<?php echo $row['author']; ?>">
            </div>
            <div class="col-md-12 col-sm-12 col-xs-12 single-form-element">
                <textarea name="content" class="form-control"><?php echo $row['content']; ?></textarea>
            </div>
            <input type="hidden" name="id" value="<?php echo $_GET['id']; ?>">
            <div class="col-md-12 col-sm-12 col-xs-12 single-form-element">
                <input type="submit" class="btn btn-primary pull-right" value="Zatwierdź/Wróć" id="form-action" style="background-color: #00c7ac !important;">
            </div>
        </form>
        <?php
    endwhile;
} else {
    echo '<div class=\"alert alert-danger\" role=\"alert\">Komentarz nie istnieje.</div>';
}
?>
<?php include '/../layout/footer.php'; ?>
